...
class MyActorManager(ActorManager):
    ...
    def omnet_init_completed(self, message):
        ...
        print(f"end generating actors: {self._carlanet_actors}")
    ...
    def generic_message(self, timestamp, message) -> (SimulatorStatus, dict):
        ...
        if message['user_message_type'] == 'ACTOR_STATUS_UPDATE':
            ...
            while any(not actor.done(CarlaClient.instance.world.get_snapshot().timestamp) for actor in self._carlanet_actors.values()):
                ...
            ...
            actor_id = message['actor_id']
            if isinstance(self._carlanet_actors[actor_id], TeleCarlaVehicle):
                if instruction_id != str(-1): self._carlanet_actors[actor_id].apply_instruction(ObjectStorage.get(instruction_id))
    ...
    def _generateVehicle(self, actor, origin):
        ...
        #set start position (getted from start origin of agent route) otherwise it start from the random spown_point
        ...
        try:
            ...
            if 'lidar' in actor and actor['lidar']:
                lidar_sensor = TeleCarlaLidarSensor(parent_actor=carlanet_actor.carla_actor)
